package corr.ai.module.system.api.oauth2;

import corr.ai.framework.common.pojo.CommonResult;
import corr.ai.module.system.api.oauth2.dto.OAuth2AccessTokenCheckRespDTO;
...
public interface OAuth2TokenApi {
    ...
    @Parameters({
        ...
        @Parameter(name = "clientId", description = "客户端编号", required = true, example = "corryuanma")
    })
    CommonResult<OAuth2AccessTokenRespDTO> refreshAccessToken(@RequestParam("refreshToken") String refreshToken,
                                                              @RequestParam("clientId") String clientId);

    @GetMapping(PREFIX+"/getToken")
    @Operation(summary = "获取已登录用户的token")
    CommonResult<String> getAccessTokenByLoginUser(Long userId);
