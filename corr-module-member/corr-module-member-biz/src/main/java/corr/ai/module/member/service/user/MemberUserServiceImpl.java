package corr.ai.module.member.service.user;
...
import corr.ai.module.member.controller.admin.user.vo.MemberUserPageReqVO;
import corr.ai.module.member.controller.admin.user.vo.MemberUserUpdateReqVO;
import corr.ai.module.member.controller.app.user.vo.*;
...
import corr.ai.module.member.dal.mysql.user.MemberUserMapper;
import corr.ai.module.member.mq.producer.user.MemberUserProducer;
...
public class MemberUserServiceImpl implements MemberUserService {
    ...
    @Override
    public void updateWalletAddress(Long userId, String address) {
        UpdateWrapper<MemberUserDO> wrapper = new UpdateWrapper<>();
        wrapper.set("wallet_address", address);
        memberUserMapper.update(wrapper);
    }
    ...
    @Override
    public void applyVipInfo(Long userId, Long vipId) {
        UpdateWrapper<MemberUserDO> wrapper = new UpdateWrapper<>();
        wrapper.set("vip_id",vipId);
    ...
    @Override
    public MemberUserDO getUserByWallet(String walletAddress) {
        return memberUserMapper.selectUserByWallet(walletAddress);
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public MemberUserDO createUserByEmailIfAbsent(String email, String password, String registerIp, Integer terminal) {
        MemberUserDO memberUserDO = memberUserMapper.selectUserByEmail(email);
        if (memberUserDO != null) {
            return memberUserDO;
        }
        return createUserByEmail(email, null, null, registerIp, password, terminal);
    }

    @Transactional(rollbackFor = Exception.class)
    @Override
    public MemberUserDO createUserByWalletIfAbsent(String walletAddress, String password, String registerIp, Integer terminal) {
        MemberUserDO memberUserDO = memberUserMapper.selectUserByWallet(walletAddress);
        if (memberUserDO != null) {
            return memberUserDO;
        }
        return createUserByWallet(walletAddress, null, null, registerIp, password, terminal);
    }
    ...
    private MemberUserDO createUserByEmail(String email, String nickname, String avtar,
                                           String registerIp, String password, Integer terminal) {
        // 生成密码
//        String password = IdUtil.fastSimpleUUID();
        // 插入用户
        MemberUserDO user = new MemberUserDO();
    ...
    private MemberUserDO createUserByWallet(String address, String nickname, String avtar, String registerIp, String password, Integer terminal) {
        MemberUserDO user = new MemberUserDO();
